...
def list_all_files(path_dir: str = './domain', ext: str = '.py', exclude=None) -> List[str]:
    files = []
    for entry in os.scandir(path_dir):
        if entry.is_dir():
            ...
            if not ext or (ext and entry.path.endswith(ext)):
                if exclude and entry.path.endswith(exclude):
                    continue
...
def gen_exports(path_dir='./domain', gen_flag='# the __all__ is generated'):
    files = list_all_files(path_dir=path_dir)
    for file in files:
        exports = []
        lines = []
...
__all__ = ['list_all_files', 'all_sub_modules', 'all_sub_all', 'gen_exports', 'gen_kdata_schema']
